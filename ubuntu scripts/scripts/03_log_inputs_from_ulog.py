...
def extract_inputs(ulog: ULog) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    # Helper: build DataFrame from timestamp + columns
    ...
    if ds_thrust is not None and ds_torque is not None:
        ...
        # thrust: try common field patterns
        # In many PX4 logs, thrust is a 3-vector "xyz[0..2]" (body thrust).
        # We'll use the Z component magnitude as "T_proxy" by default.
        thr_x = col_any(td, "xyz[0]", "xyz[0]")
        thr_y = col_any(td, "xyz[1]", "xyz[1]")
        thr_z = col_any(td, "xyz[2]", "xyz[2]")
        if thr_z is None:
            # fallback: sometimes named "thrust_body[2]" or "thrust[2]"
            thr_z = col_any(td, "thrust_body[2]", "thrust[2]", "thrust_z")

        tau_x = col_any(qd, "xyz[0]", "xyz[0]", "torque[0]", "torque_body[0]")
        tau_y = col_any(qd, "xyz[1]", "xyz[1]", "torque[1]", "torque_body[1]")
        tau_z = col_any(qd, "xyz[2]", "xyz[2]", "torque[2]", "torque_body[2]")
...
def main():
    ...
    ap.add_argument("--ulg", type=str, default="", help="Path to .ulg (optional). If empty, auto-find newest.")
    ...
    ulg_path = Path(args.ulg).expanduser().resolve() if args.ulg else None
